...
namespace Algebra{
  ...
  capd::interval Series::normHs(interval q){
    interval sum(0);
    int n = get_n(this->type,this->mainSize);
    for(int i=0;i<main.dimension();i++){
      if( interval(0).subset(main[i]) ){
        sum += interval(0,1)*power(abs(main[i]).right()* power(getSeriesIndex(this->type,i) ,q) ,interval(2));
      }
      else{
        sum += power(abs(main[i])* power(getSeriesIndex(this->type,i) ,q) ,interval(2));
      }
;
    }
    interval rest = interval(0,1)* (this->C.right() * (interval(n)^(-2*(this->s -q) + interval(1)))/(2*(this->s-q) - interval(1)));
    if(interval(0).subset(sum ) ){
      return ((sum + rest).right())^interval(1./2.);
    }
    return (sum + rest)^interval(1./2.); //+ interval(0,1)* this->C.right() * ((interval(n)^(-2*(this->s -q) + interval(1)))/(2*(this->s-q) - interval(1)));
  }
  ...
  Series Series::refineTail(interval sNew) 
  {
    if(sNew < s ){
      ...
      throw debugException("Can't refine Tail\n");
    }
  }
  ...
  interval Series::getNewC(interval sNew) const
  {
    if(sNew < this->s ){
      ...
      throw debugException("Can't fit new s into series.\n");
    }
  }
  ...
  bool Series::subset(const Series& x) 
  {
    if(! (interval(0).subset(this->C)) && (interval(0).subset(this->C)) ){
      throw debugException("One of the series does't contain zero\n");
    }
    if(this->type != x.type){
      throw debugException("Type error\n");
    }
  ...
  Series Series::seriesDerivative() 
  {
    Series z;
    IVector main;
    switch(type){
      case SeriesType::sin  : 
        main = IVector(this->mainSize);
        for(int i=0;i<this->mainSize;i++){
          main[i] = this->main[i]*(i+1);
        }
        z = Series(main,this->C,this->s-1,SeriesType::cos);
        break;
      case SeriesType::cos  : 
        main = IVector(this->mainSize-1);
        for(int i=0;i<this->mainSize-1;i++){
          main[i] = (-1)*this->main[i+1]*(i+1);
        }
        z = Series(main,(-1)*this->C,this->s-1,SeriesType::sin);
        break;
      default:
        throw debugException("Type of Series can not be SinOdd or CosEven.\n");
    }    
    return z;
  }
  ...
  Series exp(const Series &x,const interval& sNew) 
  {
    ...
    if(x.s >= 0 || x.C.right()>=0){
      std::cout<<x.C<<"\n";
      std::cout<<x.s<<"\n";
      throw debugException("exp - operation not supported for x Series\n");
    }
  ...
  Series expMinusOne(const Series &x) 
  {
    ...
    if(x.s > 0){
      ...
      if(x.C > interval(0))
      {
        throw debugException("ExpMinusOne error series in not decaying and is positive");
      }
      else{
        y.s = interval(0);
        y.C = interval(-1,0);
  ...
  Series intersection(const Series&x ,const Series&y) 
  {
    ...
    for(int i=0;i<newSize;i++){
      ...
      if(!intersection(x.valueAt(k),y.valueAt(k),newMain[i]))
      {
        ...
        x.print();
        y.print();
        throw debugException("Series intersection is empty!");
  ...
  Series static sinTimesCos(const Series& xSin,const Series& yCos){
    ...
    if(yCos.s<=interval(1)){
        throw debugException("Cos Series does not have required decay\n");
    }
    if(xSin.s<=interval(1)){
      if(xSin.s<=interval(0) && (yCos.s+xSin.s)<=interval(1) ){
        std::cout<< "SinDecay: "<<xSin.s<<"\n";
        std::cout<< "CosDecay: "<<yCos.s<<"\n";
        throw debugException("Series does not have required decay\n");
      }
      if(xSin.s> interval(0) &&  (yCos.s-xSin.s)<=interval(1) ){
        std::cout<< "SinDecay: "<<xSin.s<<"\n";
        std::cout<< "CosDecay: "<<yCos.s<<"\n";
    ...
    if(xSin.mainSize + 1 != yCos.mainSize){
        throw debugException("sinTimesCos Size error\n");
    }
  ...
  Series static sinTimesSin(const Series& x,const Series& y){
    ...
    interval estimationsMain = ((abs(x.C* y.C)) / (interval(1)*(x.s+y.s) -interval(1) ) ) * (interval(n)^(interval(-1)*(x.s+y.s) + interval(1) ) );
    ...
    interval estimationTail;
    if(s>1){
    estimationTail = abs(xC*yC) * (interval(2)^s + 1) * 
                              (interval(1) /(interval(1)*s - interval(1))) * (interval(n)^(interval(-1)*s + interval(1) ) );
    }
    else if(s>=0 ){
       //Oszacowanie konwolucji y_{i} x_{k+i} i y_{i+k} x_{k}
        estimationTail+= x.C*y.C * ((interval(n)^interval( - y.s + 1))/interval(-1 + y.s));
        //Oszacowanie konwolucji y_{i} x_{k-i}
        estimationTail+= (x.C*y.C * ((interval(2)/interval(n+1))^x.s))* (interval(n)^(x.s - y.s + 1))/interval(-1 - x.s + y.s);  
    }
    result.main = newMain;
    interval C;
  ...
  {
    if(x.vec.size() != y.vec.size())
      throw debugException("Size intersection error\n");
    SeriesVector z(x.vec.size());
    for(int i=0;i<x.vec.size();i++){
  ...
  {
    if(x.vec.size()!=y.vec.size())
      throw debugException("wrong sizes!\n");
    SeriesVector z(x.vec.size());
    for(int i=0;i<x.vec.size();i++){
