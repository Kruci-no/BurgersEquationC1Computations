...
void Mover::move(Set& x,VectorField& vectorField,bool constStep = false) 
{
    ...
    */interval x_Eval1Sum = interval(0);
    ...
    for(int i=0; i<x.vector.vec.size();i++){
        ...
        Series x_Eval2 = elementWiseMult(exp(dtL[i], -vectorField.L[i].s*(1./2)) ,x.vector[i]) + nonLinearDuhamelTerm[i];
        ...
        if( (x_Eval1Sum < x_Eval2Sum) || ( (abs(x_Eval2.C)).right() >= 0.01) || x_Eval2.s >= maxDecay ){
            x.vector[i] = x_Eval1;   
            if((abs(x.vector[i].C)).right() >100 && x.vector[i].s>minDecay + interval(1./2)){
              x.vector[i]  = x.vector[i].refineTail(x.vector[i].s-interval(1./2)) ; 
            }  
        }
        else{
            x.vector[i] = x_Eval2;
