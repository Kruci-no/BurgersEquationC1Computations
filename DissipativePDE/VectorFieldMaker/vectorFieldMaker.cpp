...
namespace VectorFieldMaker{
	...
	std::vector<string> sinSqueredDerivative(std::vector<string> u, int maxSize){
		auto r = sinSquered(u,maxSize+1);
		auto result = r;
		result.resize(maxSize);
		for(int i=0;i<maxSize;i++){
			result[i] = "(("+to_string(-i-1)+")"+"*("+r[i+1]+"))";
		}
		return result;
	}
	
	std::vector<string> sinSqueredDerivative(std::vector<string> u){
		return sinSqueredDerivative(u,2*u.size());
	}
	...
	std::vector<string> sinTimesSinDerivative(std::vector<string> u1,std::vector<string> u2,int maxSize){
		auto r = sinTimesSin(u1,u2,maxSize+1);
		auto result = r;
		result.resize(maxSize);
		for(int i=0;i<maxSize;i++){
			result[i] = "(("+to_string(-i-1)+")"+"*("+r[i+1]+"))";
	...
	std::vector<string> sinTimesSinDerivative(std::vector<string> u1,std::vector<string> u2){
		return sinTimesSinDerivative(u1,u2,u1.size() +u2.size());
	}
